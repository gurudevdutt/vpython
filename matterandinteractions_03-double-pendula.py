...
class pendulum:
# theta1 initial upper angle (CCW from vertical)
# theta1dot initial rate of change of theta1
# theta2 initial lower angle (CCW from vertical)
# theta2dot initial rate of change of theta2
# pos is in front of the pedestal if only one pendulum shown; a second pendulum is behind the pedestal
    def __init__(self, pos=topofbar, theta1=1, theta1dot=0, theta2=0, theta2dot=0):
        ...
        c = cylinder(pos=pos+vector(L1,0,-(gap+d)/2), axis=vector(0,0,gap+d), radius=axle.radius, color=color.green)
        self.frame1 = compound([b1,b2,c])
        self.frame1.rotate(angle=self.initial_theta1-pi/2, axis=vector(0,0,1), origin=vector(0,0,0))
        self.frame1.initial_pos = vector(self.frame1.pos) # keep a copy
        self.frame1.initial_axis = vector(self.frame1.axis) # keep a copy
